...
class CM1KEmulator:
    ...
    def __init__(self, network_size=0):
        ...
        # self.log=("New RBFNetwork created")
        # self.col_log.extend(self.log)
        # self.dump_registers()
    ...
    def __repr__(self):
        ...
        # self.log=("{:12} {:>2}: {:>10} {:>10}".format(key, val[0], self.registers[val[0]], "0x{:X}".format(self.registers[val[0]])))
        # self.col_log.extend(self.log)
    ...
    def learn(self, input_, cat, gcr=None):
        ...
        min_firing_dist = self.read_maxif()
        ...
        for neuron in self.firing_neurons:
            neuron.shrink_if_necessary(cat, neuron.dist, minif)
        ...
        # self.log = ("any_correctly_firing_neuron: {}".format(any_correctly_firing_neuron))
        # self.col_log.extend(self.log)
        for neuron in self.neurons:
            if neuron.state == nrn.NeuronState.com:
                #(neuron.aif)
                if min_firing_dist > neuron.dist:
                    min_firing_dist = neuron.dist
            else:
                break
        # for neuron in self.neurons:
        ...
        if not any_correctly_firing_neuron:
            ...
            if rtl_neuron_idx is not None:
                # print("rtl_neuron: idx{}, id{}".format(rtl_neuron_idx,
                #                                        self.neurons[rtl_neuron_idx].id_ if self.neurons[
                #                                            rtl_neuron_idx] else None))
                new_aif = min(max(min_firing_dist, minif), self.read_maxif())

                #print("new_aif: {}".format(new_aif))
                self.neurons[rtl_neuron_idx].commit(self.read_gcr_context(), cat, new_aif, input_)
                # print(
                #     "Committed: id{} st{}".format(self.neurons[rtl_neuron_idx].id_, self.neurons[rtl_neuron_idx].state))
                ...
                print("RTL neuron is None")
        return self.neurons

    def learn_write_all(self, input_, cat, gcr=None):
        """
        input_ of len 1-256 (limit to 256 to properly emulate a CM1K).
        cat 0-32767. 1-32767: vector category. 0: counterexample (shrink aifs, but don't commit RTL neuron).
        gcr 0-127. 1-127: context. 0: use all neurons (disregard context). None: don't overwrite current context.
